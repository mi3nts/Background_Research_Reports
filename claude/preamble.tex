...
% 2026-08-08: the DOI line was breaking away from its entry and stranding itself alone on
% an otherwise empty page. Neither \\* nor \nopagebreak stopped it, and trimming prose does
% not converge (the reflow simply pushes the next line down). Fixed the way \precisentry
% below already was: the whole entry is a minipage, so it moves as one block. Layout,
...
% ---- rollup digest tiers -------------------------------------------------------
% Promoted from build/monthly.tex on 2026-08-02: mkdigest.py emits \subsectionnote and
% \precisentry for EVERY cadence, so defining them only in the monthly meant the first
% weekly that used the digest failed to compile with 40 undefined-control-sequence
% errors. They belong in the shared design system, not in one issue.
% A tier label inside a band.
\providecommand{\subsectionnote}[1]{%
  \vspace{0.6mm}\noindent{\sffamily\fontsize{7.4}{9}\selectfont\color{Slate}\textsc{#1}}\par
  \vspace{1.0mm}}
% Compact entry for a record with no assessed summary. Deliberately lighter than
% \paperentry so a reader can tell an assessed summary from an unread record at a glance.
